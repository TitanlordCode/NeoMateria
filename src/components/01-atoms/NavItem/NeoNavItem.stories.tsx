...
import { navItemSizes } from './NeoNavItemTypes'
...
const meta: Meta<typeof NeoNavItem> = {
	...
	argTypes: {
		...
		showConnector: {
			...
			table: { category: 'Appearance' },
			description:
				'Draws a vertical connector line on the left edge to visualise hierarchy between sibling items.',
		},
		active: {
			control: 'boolean',
			table: { category: 'State' },
			description:
				'Marks this item as representing the current page. Sets `aria-current="page"` and applies active styling.',
		},
		expanded: {
			control: 'boolean',
			table: { category: 'State' },
		...
		level: {
			...
			table: { category: 'Behavior' },
			description:
				'Nesting depth. Each level adds indentation. Used to build tree-style navigation hierarchies.',
		},
		hasChildren: {
			control: 'boolean',
			table: { category: 'Behavior' },
			description:
				'Indicates the item has collapsible child items. Renders a chevron and changes the element from `<a>` to `<button>` regardless of whether `href` is set.',
		},
	},
	args: {
	...
	render: (args: NeoNavItemProps) => {
		return defineComponent({
			...
			setup() {
				return () => (
					<div
						style={{ display: 'flex', flexDirection: 'column', gap: '8px', inlineSize: '280px' }}
					>
					...
					},
					{
						...
						children: [
							...
							{ label: 'Categories', href: '/products/categories' },
							{
								...
								label: 'Inventory',
								children: [
									{ label: 'Stock Levels', href: '/products/inventory/stock' },
									{ label: 'Reorder', href: '/products/inventory/reorder' },
								],
							},
						],
					},
					...
					return (
						<div key={item.id ?? item.label}>
							<NeoNavItem
								...
								expanded={isExpanded}
								onToggle={item.id ? () => toggleItem(item.id!) : undefined}
								v-slots={
									item.icon
										? {
			...
			setup() {
				...
				const navData: NavItem[] = [
								...
								children: [
									{
										...
										label: 'Atoms',
										children: [
											{ id: 'button', label: 'Button', href: '/docs/components/atoms/button' },
											{ id: 'input', label: 'Input', href: '/docs/components/atoms/input' },
											{ id: 'badge', label: 'Badge', href: '/docs/components/atoms/badge' },
										],
									},
									{
										id: 'molecules',
										label: 'Molecules',
										children: [
											{ id: 'card', label: 'Card', href: '/docs/components/molecules/card' },
											{ id: 'select', label: 'Select', href: '/docs/components/molecules/select' },
										],
									},
									{
										id: 'organisms',
										...
										children: [
											...
											{ id: 'footer', label: 'Footer', href: '/docs/components/organisms/footer' },
										],
									},
								],
							},
					...
					},
					{
						...
						label: 'API Reference',
						children: [
							{ id: 'props', label: 'Props', href: '/api/props' },
							{ id: 'events', label: 'Events', href: '/api/events' },
						],
					},
				]
			...
			setup() {
				...
				return () => (
					<div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap' }}>
						{sizes.map((size) => (
							<div key={size} style={{ inlineSize: '280px' }}>
								<h4 style={{ marginBlockEnd: '8px', textTransform: 'capitalize' }}>{size}</h4>
