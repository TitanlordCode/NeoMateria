...
- **npm** v9 or higher
- **Docker** — required for snapshot tests (deterministic Linux rendering) and optional for the containerised Storybook dev server
...
If you need the Docker-containerised dev server (matches the snapshot env):

```bash
npm run dev:docker   # rebuilds the image, then runs Storybook in the container
```

Day-to-day iteration should use `npm run dev`. On Windows, bind-mount I/O makes the Docker dev server 50–100× slower than native; only use `dev:docker` when you specifically need environment parity with snapshot tests.
